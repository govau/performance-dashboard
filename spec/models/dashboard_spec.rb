...
RSpec.describe Dashboard, type: :model do
  ...
  subject!(:dashboard)    { FactoryBot.create(:dashboard) }
  ...
  describe 'url' do
    ...
    subject(:dashboard) { FactoryBot.build(:dashboard, :url => '<BLAH>') }
  ...
  describe 'kpi widgets' do
    let!(:widget) { FactoryBot.create(:widget, :name => 'User satisfaction', :dashboard => dashboard) }
  ...
  describe 'last_updated' do
    before do
      FactoryBot.create(:widget, :dashboard => dashboard, :last_updated_at => '2030-01-01')
      FactoryBot.create(:widget, :dashboard => dashboard)
    end
  ...
  describe '#published?' do
    ...
    context 'published_at is in the future' do
      let(:dashboard) { FactoryBot.create(:dashboard, published_at: 5.days.from_now) }
      it { is_expected.to be false }
    end
  end

  describe 'rows' do
    let!(:widget_0_0) { FactoryBot.create(:widget, dashboard: dashboard, row: 0, pos: 0, name: Widget::KPIS[0]) }
    let!(:widget_0_1) { FactoryBot.create(:widget, dashboard: dashboard, row: 0, pos: 1, name: Widget::KPIS[1]) }
    let!(:widget_1_0) { FactoryBot.create(:widget, dashboard: dashboard, row: 1, pos: 0) }
    let!(:widget_1_1) { FactoryBot.create(:widget, dashboard: dashboard, row: 1, pos: 1) }
    let!(:widget_2_0) { FactoryBot.create(:widget, dashboard: dashboard, row: 2, pos: 0) }
  ...
  describe 'datasets' do
    subject(:dashboard) { FactoryBot.create(:dashboard_with_widgets) }
