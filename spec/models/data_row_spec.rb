...
RSpec.describe DataRow, type: :model do
  ...
  describe '::within_interval' do 
    let!(:dp_before)    { FactoryGirl.create(:data_row, row_date: 1.day.ago ) }
    let!(:dp_beginning) { FactoryGirl.create(:data_row, row_date: Date.today ) }
    let!(:dp_middle)    { FactoryGirl.create(:data_row, row_date: 3.days.from_now ) }
    let!(:dp_end)       { FactoryGirl.create(:data_row, row_date: 6.days.from_now ) }
    let!(:dp_after)     { FactoryGirl.create(:data_row, row_date: 7.days.from_now) }
  ...
  describe '#set_value_for' do 
    ...
    let(:row) { FactoryGirl.create :data_row, datasets: [dataset] }
  ...
  describe 'validations' do
    ...
    describe 'frequency' do
      let(:data_table) { FactoryGirl.create :data_table, period: period }
      let!(:other) { FactoryGirl.create :data_row, data_table: data_table,
        row_date: other_date }
      subject { FactoryGirl.build :data_row, data_table: data_table,
