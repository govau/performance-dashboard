...
export const createSlice = ({
  ...
  return (dispatch, getState, api) => {
    return api(`widgets/${widget_id}/slices/${period}/${periodStartSegment}`, getState().currentUser.token, {
      method: 'POST',
      body: JSON.stringify(formData)
    }).then(response => {
        if (!response.slice || !response.widget || !response.datasets || response.datasets.length <= 0) {
