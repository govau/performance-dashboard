...
describe('(Helpers) Widgets - widgetsHelpers', () => {
  ...
  if (typeof fixtureWidgets === 'undefined' || fixtureWidgets.length <= 0) throw new Error('invalid fixtureWidgets');
  ...
  describe('updateWidgetInWidgets', () => {
    ...
    it('should have updated the correct property on the correct widget in state', () => {
      expect(actual.find(w => w.id == newWidgetState.id).description).toEqual(newWidgetState.description);
    });
  });
