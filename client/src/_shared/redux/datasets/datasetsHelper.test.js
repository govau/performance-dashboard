...
import { getHumanisedUnits, updateDatasetInDatasets } from './datasetsHelper';
...
describe('(Helpers) Datasets - datasetsHelper', () => {
  ...
  describe('getHumanisedUnits', () => {
    ...
    it('should throw if an unrecognised value is provided', () => {
      expect(() => getHumanisedUnits('__*^Q#')).toThrow();
      expect(() => getHumanisedUnits('sdsajdajhsgdjhas')).toThrow();
    });
  });
  ...
  describe('updateDatasetInDatasets', () => {
    ...
    it('should have updated the correct property on the correct dataset in state', () => {
      expect(actual.find(d => d.id == newDatasetState.id).label).toEqual(
        newDatasetState.label,
      );
    });
  });
});
