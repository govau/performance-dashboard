...
const Summary = ({errors, headingText}) => {
  ...
  return (
    <div className={`UIK-form-validation-summary UIK-alert alert alert-danger`} role="alert">
      <div className="alert__icon">
        <Icon name="error" size="31" />
      </div>
      <div className="alert__text">
        <span className="heading-text h6">{headingText || getHeadingText()}</span>
        <ol className={`list-ordered ${errors.length === 1 ? 'has-singular' : ''}`}>{
          errors.map((error, i) => {
            return (
              <li key={i}>{
                error
