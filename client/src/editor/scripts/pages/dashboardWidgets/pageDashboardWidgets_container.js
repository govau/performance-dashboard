...
const mapStateToProps = (state, ownProps) => {
  ...
  const dashboardWidgets = selectWidgetsByDashboard(state, {
    dashboardId: dashboard.id
  });
  const ui = selectUi(state);

  // get 1 most recent denormalized slices per widget
  const denormalizedSlices = dashboardWidgets
    .filter(widget => {
      return widget.type !== "fact";
    })
    .map(widget =>
      getDenormalizedSlice(state, {
        widgetId: widget.id,
        dashboardId: dashboard.id
      })
    );
  ...
  const denormalizedFacts = dashboardWidgets
    .filter(widget => {
      return widget.type === "fact";
    })
    .map(widget => {
