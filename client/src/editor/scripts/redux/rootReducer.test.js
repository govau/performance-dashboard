...
describe('(Reducers) Root - rootReducer', () => {
  ...
  describe('Cross cutting on Slice', () => {
    ...
    describe('HYDRATE__SLICE_UPDATED', () => {
      ...
      beforeAll(() => {
        payload = {
          ...
          slice: {...fixtureSlice, ...{widget_id:fixtureWidget.id, groups:[{dataset_id:fixtureDataset.id, value: '123'}]}},
          datasets: [{...fixtureDataset, ...{value:'123'}}]
        };
        actual = reducer({...initialState, ...fixtureState}, {type: sliceTypes.HYDRATE__SLICE_UPDATED, payload});
        actualWidget = actual.widgets.find(w => w.id == fixtureWidget.id);
        actualSlice = actual.slices.find(s => {
          return s.widget_id == fixtureWidget.id &&
      ...
      it('should not change state slice length', () => {
        expect(actual.slices.length).toEqual({...initialState, ...fixtureState}.slices.length);
      });
    ...
    describe('HYDRATE__SLICE_CREATED', () => {
      ...
      beforeAll(() => {
        payload = {
          widget: {...fixtureWidget, ...{description: 'cats', datasets:[fixtureDataset.id]}},
          slice: {
        ...
        actual = reducer({...initialState, ...fixtureState}, {type: sliceTypes.HYDRATE__SLICE_CREATED, payload});
        actualWidget = actual.widgets.find(w => w.id == fixtureWidget.id);
        actualSlice = actual.slices.find(s => {
          return s.widget_id == payload.widget.id &&
      ...
      it('should increase the size of slices', () => {
        expect(actual.slices.length).toEqual({...initialState, ...fixtureState}.slices.length + 1)
      });
