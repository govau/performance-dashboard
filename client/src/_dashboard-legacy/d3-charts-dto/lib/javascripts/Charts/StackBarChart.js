...
class StackBarChart extends BarChart {
  ...
  calculateYMin() {
    ...
    let min =
      d3.min(this.transformedData.map(c => d3.min(c, d => d.y0))) -
      this.padding.bottom;
  ...
  calculateYMax() {
    ...
    let max =
      d3.max(this.transformedData.map(c => d3.max(c, d => d.y0 + d.y))) +
      this.padding.top;
  ...
  getXScale() {
    return d3.scale
      ...
      .domain(this.transformedData[0].map(d => d.x));
  }
      ...
      d3.select(this)
        ...
        .attr('fill', (d, i) => {
          if (that.isHighContrastMode === true) {
            return isActive(d, i, j) === true
              ? d.altColorDark || d3.rgb(d.color).darker(0.85)
              : d.altColor;
          } else {
            return isActive(d, i, j) === true
              ? d3.rgb(d.color).darker(0.8)
              : d.color;
          }
        });
    });
